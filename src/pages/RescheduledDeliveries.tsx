...
export const RescheduledDeliveries: React.FC = () => {
  const { user } = useAuth();
      ...
      (snapshot) => {
        const deliveriesData = snapshot.docs.map(doc => {
          ...
          return {
            ...
            rescheduledAt: data.rescheduledAt?.toDate() || new Date(),
            originalScheduledTime: data.originalScheduledTime?.toDate() || new Date(),
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900">Livraisons Reprogrammées</h1>
            <p className="mt-1 text-sm text-gray-500">
              Liste des livraisons dont la date a été modifiée
            </p>
          </div>
          <DeliveryList
